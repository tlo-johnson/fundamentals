...
char* catAndMouse(int catA, int catB, int mouse) {
  int deltaA = catA < mouse ? mouse - catA : catA - mouse;
  int deltaB = catB < mouse ? mouse - catB : catB - mouse;

  char* result = malloc(10 * sizeof(char));
  if (deltaA < deltaB) result = "Cat A";
  else if (deltaA == deltaB) result = "Mouse C";
  else result = "Cat B";
