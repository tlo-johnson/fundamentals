...
int getMoneySpent(int keyboards_count, int* keyboards, int drives_count, int* drives, int b) {
  ...
  for (int k_count = 0; k_count < keyboards_count; k_count++) {
    int keyboard_cost = keyboards[k_count];
    for (int d_count = 0; d_count < drives_count; d_count++) {
      int total = keyboard_cost + drives[d_count];
      if (total > maxSum && total <= b) {
        maxSum = total;
      }
    }
  }
