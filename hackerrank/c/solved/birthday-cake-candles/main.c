...
int birthdayCakeCandles(int candles_count, int* candles) {
  int maxHeight = candles[0], numMaxHeightCandles = 1;

  for (int count = 1; count < candles_count; count++) {
    int height = candles[count];
    if (height > maxHeight) {
      maxHeight = height;
      numMaxHeightCandles = 1;
    } else if (height == maxHeight) {
      numMaxHeightCandles++;
    }
  }

  return numMaxHeightCandles;
}
