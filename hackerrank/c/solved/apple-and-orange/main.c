...
void countApplesAndOranges(int houseStart, int houseEnd, int appleLocation, int orangeLocation, int apples_count, int* apples, int oranges_count, int* oranges) {
  int appleResult = 0;
  for (int count = 0; count < apples_count; count++) {
    int end = apples[count] + appleLocation;
    if (end >= houseStart && end <= houseEnd) appleResult++;
  }

  int orangeResult = 0;
  for (int count = 0; count < oranges_count; count++) {
    int end = oranges[count] + orangeLocation;
    if (end >= houseStart && end <= houseEnd) orangeResult++;
  }
