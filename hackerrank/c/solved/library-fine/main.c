...
int libraryFine(int returnDay, int returnMonth, int returnYear, int dueDay, int dueMonth, int dueYear) {
  if (returnYear > dueYear) return 10000;
  if (returnMonth > dueMonth && returnYear == dueYear) return 500 * (returnMonth - dueMonth);
  if (returnDay > dueDay && returnMonth == dueMonth && returnYear == dueYear) return 15 * (returnDay - dueDay);
  return 0;
}
