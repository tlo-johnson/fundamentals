 ...
 *
 */
int* serviceLane(int width_count, int* width, int cases_rows, int cases_columns, int** cases, int* result_count) {
  int* results = malloc(cases_rows * sizeof(int));

  for (int case_index = 0; case_index < cases_rows; case_index++) {
    int start = cases[case_index][0];
    int end = cases[case_index][1];

    int max = 0;
    for (int width_index = start; width_index <= end; width_index++)
      if (max == 0 || max > width[width_index])
        max = width[width_index];

    results[case_index] = max;
  }
